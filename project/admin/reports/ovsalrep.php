...
<body>
    ...
    <div class="container">
        ...
        $sql = "SELECT `Emp_Id`, `_date`, `overtime_salary` FROM `osalary` WHERE `_date`='$d'";
        $res = mysqli_query($con, $sql);

        if (!$res) {
            echo "<div class='container'><p class='alert alert-danger mt-5'>Error: " . mysqli_error($con) . "</p></div>";
            ...
            echo "<thead><tr><th>Employee Name</th><th>Date</th><th>Overtime Salary</th></tr></thead>";
            echo "<tbody>";
            while ($x = mysqli_fetch_array($res)) {
                ...
                echo "<tr><td>{$name['cust_name']}</td><td>{$x['_date']}</td><td>{$x['overtime_salary']}</td></tr>";
            }
